...
class _SelectionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    _selectionDecoration ??= BoxDecoration(
      ...
      border: Border.all(color: _calendarTheme.selectionBorderColor, width: 2),
    ...
    if (!_isTimeline) {
      if (_calendar.view == CalendarView.month) {
        if (_selectedDate != null) {
          if (isDateWithInDateRange(_visibleDates[0],
              _visibleDates[_visibleDates.length - 1], _selectedDate)) {
            for (int i = 0; i < _visibleDates.length; i++) {
              ...
              if (isSameDate(date, _selectedDate)) {
                ...
                if (_isRTL) {
                  _xPosition =
                      (_numberOfDaysInWeek - 1 - _rowIndex) * _cellWidth;
                } else {
        ...
        if (_appointmentView != null && _appointmentView.appointment != null) {
          ...
          if (isSameDate(_visibleDates[0], _selectedDate)) {
            if (_isRTL) {
              _xPosition = 0;
            } else {
        ...
        if (_appointmentView != null && _appointmentView.appointment != null) {
          ...
          if (isDateWithInDateRange(_visibleDates[0],
              _visibleDates[_visibleDates.length - 1], _selectedDate)) {
            for (int i = 0; i < _visibleDates.length; i++) {
              if (isSameDate(_selectedDate, _visibleDates[i])) {
                _rowIndex = i;
                if (_isRTL) {
                  _xPosition =
                      _cellWidth * (_visibleDates.length - 1 - _rowIndex);
                } else {
